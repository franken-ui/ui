...
export default function (variables: { [key: string]: { [key: string]: string } }) {
	const palette: { [key: string]: { [key: string]: string } } = {};
	const hexes: { [key: string]: { [key: string]: string } } = {};
	...
	for (const [theme, colors] of Object.entries(variables)) {
		...
		for (const [colorKey, colorValue] of Object.entries(colors)) {
			const [h, s, l] = colorValue.split(' ').map((a: string) => Number(a.replace('%', '')));
		...
		palette[theme] = {
			...variables[theme],
			'--uk-form-checkbox-image': `url("data:image/svg+xml,%3Csvg width='15' height='15' viewBox='0 0 15 15' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='100%25' height='100%25' fill='${encodeURIComponent(
				hexes[theme]['--primary']
				...
				hexes[theme]['--primary-foreground']
			)}' fill-rule='evenodd' clip-rule='evenodd' %3E%3C/path%3E%3C/svg%3E")`,
			'--uk-form-checkbox-image-indeterminate': `url("data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%2216%22%20height%3D%2216%22%20viewBox%3D%220%200%2016%2016%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%0A%20%20%20%20%3Crect%20fill%3D%22${encodeURIComponent(
				hexes[theme]['--primary']
			...
			'--uk-form-list-image': `url("data:image/svg+xml,%3Csvg width='24' height='16' viewBox='0 0 24 16' xmlns='http://www.w3.org/2000/svg'%3E%3Cpolygon fill='${encodeURIComponent(
				hexes[theme]['--muted-foreground']
			...
			'--uk-stepper-check-image': `url("data:image/svg+xml,%3Csvg width='15' height='15' viewBox='0 0 15 15' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M11.4669 3.72684C11.7558 3.91574 11.8369 4.30308 11.648 4.59198L7.39799 11.092C7.29783 11.2452 7.13556 11.3467 6.95402 11.3699C6.77247 11.3931 6.58989 11.3355 6.45446 11.2124L3.70446 8.71241C3.44905 8.48022 3.43023 8.08494 3.66242 7.82953C3.89461 7.57412 4.28989 7.55529 4.5453 7.78749L6.75292 9.79441L10.6018 3.90792C10.7907 3.61902 11.178 3.53795 11.4669 3.72684Z' fill='${encodeURIComponent(
				hexes[theme]['--primary']
			)}' fill-rule='evenodd' clip-rule='evenodd' %3E%3C/path%3E%3C/svg%3E%0A")`,
